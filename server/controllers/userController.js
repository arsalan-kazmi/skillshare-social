...
const deleteEducation = async (req, res) => {
  try {
    ...
    console.log(
      "Current education IDs:",
      user.education.map(e => e._id.toString())
    );
...
const addExperience = async (req, res) => {
  try {
    ...
    if (!company || !title) {
      return res
        .status(400)
        .json({ message: "Company and title are required" });
    }
...
const updateExperience = async (req, res) => {
  ...
  try {
    ...
    if (employmentType)
      user.experience[expIndex].employmentType = employmentType;
...
const updateInternships = async (req, res) => {
  try {
    ...
    if (description)
      user.internships[internshipIndex].description = description;
    await user.save();
...
const deleteLanguage = async (req, res) => {
  try {
    ...
    user.languages = user.languages.filter(
      lang => lang._id.toString() !== req.params.langId
    );
    await user.save();
    res.json({
      success: true,
...
const updateLanguage = async (req, res) => {
  ...
  try {
    ...
    const langIndex = user.languages.findIndex(
      language => language._id.toString() === req.params.langId
    );
    if (langIndex == -1) {
      return res.status(404).json({
...
const addSkill = async (req, res) => {
  try {
    ...
    if (!name || !level) {
      return res
        .status(400)
        .json({ message: "Skill name and Level is required" });
    }
